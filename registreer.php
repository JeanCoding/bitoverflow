  ...
  <div>
    <div style="background-color: #242424;" class="lg:w-5/12 h-full lg:fixed right-0 top-0 pl-1 z-0">
      ...
      <form method='POST' class="flex flex-col items-center mt-10">
        <div class="grid gap-4 lg:grid-cols-2 ">
          <div style="justify-content: space-between;">
            ...
            <input type="text" name='first_name' class="rounded-full lg:px-3 py-2 pl-4 text-white outline-none ml-1" style="background-color: #3D3D3D; font-family: Poppins;" placeholder="VOORNAAM">
          </div>
          <div>
            <p style="font-family: Laro;" class="ml-2 mb-2 text-white">ACHTERNAAM:</p>
            <input type="text" name='last_name' class="rounded-full lg:px-3 px-1 py-2 pl-4 text-white outline-none" style="background-color: #3D3D3D; font-family: Poppins;" placeholder="ACHTERNAAM">
          </div>
          <div>
            <p style="font-family: Laro;" class="ml-2 mb-2 text-white">E-MAIL:</p>
            <input type="email" name='email' class="rounded-full lg:px-3 px-1 py-2 pl-4 text-white outline-none" style="background-color: #3D3D3D; font-family: Poppins;" placeholder="E-MAIL">
          </div>
          <div>
              <p style="font-family: Laro;" class="ml-2 mb-2 text-white">WACHTWOORD:</p>
              <div class="flex">
              <input type="password" name='password' id="myInput" class="rounded-full lg:px-3 px-1 py-2 pl-4 text-white outline-none" style="background-color: #3D3D3D; font-family: Poppins;" placeholder="WACHTWOORD">
      ...
      <div class="flex justify-center">
        <p class="mb-6 absolute lg:bottom-0 text-white cursor-default pb-6 lg:text-base text-sm" style="font-family: Laro;">HEB JE AL EEN ACCOUNT? <a href="login.php" class="text-sky-500 hover:text-sky-700 ease-in-out duration-300" style="font-family: Laro;">LOGIN</a></p>
      </div>
</body>
</html>
...
if (isset($_POST['submit'])) {
  ...
  if ($row['num'] > 0) {
    ...
    $sql = "INSERT INTO `users` (`first_name`, `last_name`, `school_year`, `email`, `password`) 
    VALUES (:first_name, :last_name, :school_year, :email, :password)";
    $sql_run = $pdo->prepare($sql);
    $result = $sql_run->execute(array(
      ":first_name" => $first_name, ":last_name" => $last_name, ":school_year" => $school_year, ":email" => $email, ":password" => $password,
    ));
