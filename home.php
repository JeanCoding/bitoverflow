    ...
    <div class='flex'>
        ...
        <?php
            ...
            <div class='mt-6 lg:mt-0 bg-neutral-700 text-white py-8 px-7 rounded-3xl flex lg:w-[900px]' style='box-shadow: 0px 4px 40px 2px rgba(0, 0, 0, 0.25);'>
                <?php
                $query = "SELECT * FROM posts INNER JOIN users ON posts.user_id = users.id INNER JOIN categories ON posts.category_id = categories.id ORDER BY posts.id DESC LIMIT 0, 1;
                ";
                $sql = $pdo->prepare($query);
                $sql->execute();
                $rows = $sql->fetchAll();
                        ...
                        <form method='POST'>
                            <div class='w-full flex justify-between items-center mt-12 text-3xl font-bold'>
                                <div class='flex'>
                                    ...
                                    <span class='w-10 h-10 lg:w-12 lg:h-12 p-2 flex items-center justify-center font-bold text-2xl rounded-full cursor-pointer' style='background: #FF5959'><img src='icons/down_arrow.svg'></span>
